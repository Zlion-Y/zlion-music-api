> [!IMPORTANT]
> **本仓库已归档（2026-09-14），不再维护。**
>
> 后来这套东西换了个更省事的形态：音源解析做成 **serverless 函数**，跟主页一起部署，不用再单独养一台后端。
> 新方案见 [**Zlion-Y/zlion-homepage**](https://github.com/Zlion-Y/zlion-homepage)（公开仓库，含 `api/` + `lib/` + `sources/`）：
>
> - 前端调**同源**的 `/api/url`，不需要额外域名、证书、CORS 配置；
> - 服务端用 Node 原生 `vm` 跑洛雪音源脚本（脚本本来就是 JS，连垫片都不需要）；
> - 多音源按成绩分波对冲、赢家一出即掐断其余、直链探活、连续失败熔断——这套调度逻辑原样保留了；
> - 直链缓存放 CDN 边缘，命中缓存的请求根本不进函数。
>
> 本仓库保留作为「自建一台后端」方案的历史记录：代码与思路仍然有效（Go 单二进制、管理页、
> 热更新、并行体检、自动熔断都是完整的），下方预编译二进制也仍在，只是不再更新了。
